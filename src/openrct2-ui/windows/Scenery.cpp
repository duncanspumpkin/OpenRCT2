...
public:
    ...
    void OnUpdate() override
    {
        ...
        else
        {
            ...
            if (!tabSelectedScenery.IsUndefined())
            {
                ...
                { // path bit
                    gCurrentToolId = static_cast<Tool>(
                        OpenRCT2::ObjectManager::GetMeta<PathBitEntry>(tabSelectedScenery.EntryIndex)->tool_id);
                }
                else
                { // small scenery
                    gCurrentToolId = static_cast<Tool>(
                        OpenRCT2::ObjectManager::GetMeta<SmallSceneryEntry>(tabSelectedScenery.EntryIndex)->tool_id);
                }
            }
        }
    }
    ...
    void OnPrepareDraw() override
    {
        ...
        if (!tabSelectedScenery.IsUndefined())
        {
            if (tabSelectedScenery.SceneryType == SCENERY_TYPE_SMALL)
            {
                ...
                auto* sceneryEntry = OpenRCT2::ObjectManager::GetMeta<SmallSceneryEntry>(tabSelectedScenery.EntryIndex);
    ...
    void Init()
    {
        ...
        {
            const auto* sceneryEntry = OpenRCT2::ObjectManager::GetMeta<SmallSceneryEntry>(sceneryId);
            if (sceneryEntry != nullptr)
            {
        ...
        {
            const auto* sceneryEntry = OpenRCT2::ObjectManager::GetMeta<PathBitEntry>(sceneryId);
            if (sceneryEntry != nullptr)
            {
...
private:
    ...
    std::pair<StringId, money32> GetNameAndPrice(ScenerySelection selectedScenery)
    {
        ...
        else
        {
            switch (selectedScenery.SceneryType)
            {
                ...
                {
                    auto* sceneryEntry = OpenRCT2::ObjectManager::GetMeta<SmallSceneryEntry>(selectedScenery.EntryIndex);
                    if (sceneryEntry != nullptr)
                    {
                ...
                {
                    auto* sceneryEntry = OpenRCT2::ObjectManager::GetMeta<PathBitEntry>(selectedScenery.EntryIndex);
                    if (sceneryEntry != nullptr)
                    {
                        price = sceneryEntry->price;
                        name = sceneryEntry->name;
    ...
    void DrawSceneryItem(rct_drawpixelinfo& dpi, ScenerySelection scenerySelection)
    {
        ...
        {
            auto* pathBitEntry = OpenRCT2::ObjectManager::GetMeta<PathBitEntry>(scenerySelection.EntryIndex);
            auto imageId = ImageId(pathBitEntry->image);
            gfx_draw_sprite(&dpi, imageId, { 11, 16 });
        }
        else
        {
            auto sceneryEntry = OpenRCT2::ObjectManager::GetMeta<SmallSceneryEntry>(scenerySelection.EntryIndex);
            auto imageId = ImageId(sceneryEntry->image + gWindowSceneryRotation);
