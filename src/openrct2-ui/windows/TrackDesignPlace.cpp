...
static void window_track_place_toolupdate(rct_window* w, rct_widgetindex widgetIndex, int32_t x, int32_t y)
{
    ...
    {
        place_virtual_track(_trackDesign.get(), PTD_OPERATION_DRAW_OUTLINES, true, get_ride(0), mapX, mapY, 0);
        return;
    }
    ...
    place_virtual_track(_trackDesign.get(), PTD_OPERATION_DRAW_OUTLINES, true, get_ride(0), mapX, mapY, mapZ);
}
...
static int32_t window_track_place_get_base_z(int32_t x, int32_t y)
{
    ...
    return z + place_virtual_track(_trackDesign.get(), PTD_OPERATION_GET_PLACE_Z, true, get_ride(0), x, y, z);
}
